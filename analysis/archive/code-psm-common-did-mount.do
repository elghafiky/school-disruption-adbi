...
foreach mount in "merapi" "kelud" {
	...
	*/
	...
	log using "$log\matching_psmcommon_`mount'.smcl", replace
	psmatch2 treatment_`mount' `covariates' if year == 2007 , common
	psgraph 
	gr export "$figures/psgraph-psmcommon-`mount'.png", replace
	pstest `covariates', scatter outlier treated(_treated) both
	gr export "$figures/pstest-psmcommon-`mount'.png", replace
	...
	* prepping variables for estimation
	...
	save "$final/ifls-psmcommon-`mount'-analysis.dta", replace
}

esttab merapi kelud using "$tables\samplesize_postmatching_psmcommon.tex", replace noobs nonum ///
...
foreach mount in "merapi" "kelud" {
	...
	log using "$log\analysis_psmcommon_`mount'.smcl", replace
	
	* main
	use "$final/ifls-psmcommon-`mount'-analysis.dta", clear
	eststo reg`mount': bootstrap, rep(1000): areg in_school c.distance##i.year##_treated `covariates' latitude longitude , robust cluster(district) absorb(pidlinkx)
	estadd scalar n = e(N)
	...
	* robustness 1: distance
	foreach x of numlist 100 80 60 40 {
		...
		_n as text _dup(59) "-"
		eststo reg`mount'`x': bootstrap, rep(1000): areg in_school c.distance##i.year##_treated `covariates' latitude longitude if distance <= `x', robust cluster(district) absorb(pidlinkx)
		estadd scalar n = e(N)
		estadd scalar r2a =  e(r2_a) 
	}
	
	* robustness 2: covariates
	eststo regcov1: bootstrap, rep(1000): reg in_school c.distance##i.year##_treated latitude longitude, robust cluster(district) 
	estadd scalar n = e(N)
	estadd scalar r2a =  e(r2_a) 
	
	eststo regcov2: bootstrap, rep(1000): reg in_school c.distance##i.year##_treated `covariates'  latitude longitude, robust cluster(district) 
	estadd scalar n = e(N)
	estadd scalar r2a =  e(r2_a) 
	
	eststo regcov3: bootstrap, rep(1000): areg in_school c.distance##i.year##_treated `covariates'   latitude longitude, robust cluster(district) absorb(pidlinkx)
	estadd scalar n = e(N)
	estadd scalar r2a =  e(r2_a) 
	...
	* heterogeneity
	...
	eststo reghet1: bootstrap, rep(1000): areg in_school c.distance##i.year##_treated#sex `covariates'  latitude longitude, robust cluster(district) absorb(pidlinkx)
	estadd scalar n = e(N)
	estadd scalar r2a =  e(r2_a) 
	
	eststo reghet2: bootstrap, rep(1000): areg in_school c.distance##i.year##_treated#urban `covariates' latitude longitude, robust cluster(district) absorb(pidlinkx)
	estadd scalar n = e(N)
	estadd scalar r2a =  e(r2_a) 
	
	eststo reghet3: bootstrap, rep(1000): areg in_school c.distance##i.year##_treated#birth_order `covariates' latitude longitude, robust cluster(district) absorb(pidlinkx)
	estadd scalar n = e(N)
	estadd scalar r2a =  e(r2_a)
	...
	* other outcomes
	...
	forval i=1/7 {
		eststo reg`i': bootstrap, rep(1000): areg `o`i'' c.distance##i.year##_treated `covariates' latitude longitude, robust cluster(district) absorb(pidlinkx)
	}
